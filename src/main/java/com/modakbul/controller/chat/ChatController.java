...
public class ChatController {
    ...
    public ChatMessageDto sendMessage(ChatMessageDto chatMessageDto) {
        ...
        ChatRoom chatRoom = chatRoomService.findById(chatMessageDto.getChatRoomId());
        Member member = memberService.findById(chatMessageDto.getMemberId());
        
        // DTO를 엔티티로 변환하고 메시지를 저장
        ChatMessageDto savedMessageDto = chatMessageService.saveMessage(chatMessageDto, chatRoom, member);

        // 저장된 메시지를 클라이언트로 전송
        messagingTemplate.convertAndSend("/topic/chatRoom/" + chatRoom.getId(), savedMessageDto);
